...
def validate_payment_number(service, number):
    """পেমেন্ট নম্বর validation - 11 digit check"""
    if service != "Binance":
...
def create_payment_config_table():
    """পেমেন্ট কনফিগারেশন টেবিল তৈরি - Serial number সহ"""
    conn = connection_pool.get_connection()
    cursor = conn.cursor()
    try:
        cursor.execute("""
...
def get_payment_with_serial(service_name):
    """সার্ভিসের নম্বর এবং সিরিয়াল পান"""
    conn = connection_pool.get_connection()
    cursor = conn.cursor()
    try:
...
def get_proxy_stock_count(brand_name):
    """নির্দিষ্ট ব্র্যান্ডের স্টক সংখ্যা পান"""
    conn = connection_pool.get_connection()
    cursor = conn.cursor()
    try:
...
def get_proxy_pricing(brand_name):
    """প্রক্সির মূল্য পান ($1/BDT format)"""
    conn = connection_pool.get_connection()
    cursor = conn.cursor()
    try:
...
def save_payment_number(message, service):
    """পেমেন্ট নম্বর সেভ করা - 11 digit validation সহ"""
    ...
    if text == "🛒 Buy Proxy":
        ...
        if not brands:
            ...
            for b in brands:
                ...
                if stock_count > 0:
                    btn_text = f"✅ {brand_name} (${price_usd}/{int(rate_bdt)} TK) - 📦 {stock_count}"
